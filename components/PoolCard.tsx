...
export default function PoolCard({ pool }: Props) {
  const progress = Math.round(
    (pool.current_participants / pool.min_participants) * 100,
  );
  const savings = Math.round(
    ((pool.individual_price - pool.target_price) / pool.individual_price) * 100,
  );
  ...
  return (
    <div className="group relative flex flex-col rounded-2xl border border-white/[0.07] bg-surface overflow-hidden hover:border-brand/30 hover:shadow-xl hover:shadow-brand/10 transition-all duration-200">
      ...
      <div className="relative h-52 bg-surface-2 overflow-hidden">
        ...
        <div
          className={`absolute top-3 left-3 flex items-center gap-1.5 border rounded-lg px-2.5 py-1 text-[11px] font-bold backdrop-blur-sm ${status.cls}`}
        >
          {status.icon}
          {status.label}
        </div>
      ...
      <div className="flex flex-1 flex-col p-5 gap-4">
        <div>
          ...
          <h3 className="text-[15px] font-bold text-lgray leading-snug">
            {pool.title}
          </h3>
        </div>
        ...
        <div className="flex items-end justify-between bg-surface-2 rounded-xl p-3 border border-white/[0.05]">
          <div>
            <p className="text-xs font-semibold text-white/30 line-through">
              ${pool.individual_price.toLocaleString()} retail
            </p>
            <p className="text-2xl font-bold text-lgray tracking-tight">
              ${pool.target_price.toLocaleString()}
            </p>
          </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-1.5 text-xs font-semibold text-white/40">
              <Users size={12} />
              <span>
                <span className="text-lgray font-bold">
          ...
          <p className="text-[11px] font-semibold text-white/25 mt-1.5">
            {Math.min(progress, 100)}% filled
          </p>
        </div>

        <Link
          href={`/pools/${pool.id}`}
          className="relative text-center z-20 mt-auto w-full py-2.5 bg-brand hover:bg-brand/85 text-white text-sm font-bold rounded-xl transition-colors"
        >
          Join Pool
        </Link>
      </div>
